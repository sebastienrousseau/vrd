...
fn main() {
    ...
    support::task_with_output("float() — 24-bit f32 in [0.0, 1.0)", || {
        let mut rng = Random::from_u64_seed(11);
        let samples: Vec<f32> = (0..6).map(|_| rng.float()).collect();
        let mut lines: Vec<_> =
    ...
    support::task_with_output("double() — 53-bit f64 in [0.0, 1.0)", || {
        let mut rng = Random::from_u64_seed(11);
        let samples: Vec<f64> = (0..6).map(|_| rng.double()).collect();
        let mut lines: Vec<_> =
