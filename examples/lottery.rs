...
fn main() {
    ...
    support::task_with_output("Draw 6/49", || {
        ...
        let mut winners: Vec<u32> =
            rng.sample(&pool, 6).into_iter().copied().collect();
        winners.sort_unstable();
        vec![format!("winners = {winners:?}")]
    });

    support::task_with_output(
        "Pick 3 finalists from 100 entries",
        || {
            let mut rng = Random::from_u64_seed(0xF1A1);
            let entries: Vec<String> =
                (1..=100).map(|i| format!("entrant-{i:03}")).collect();
    ...
    support::task_with_output(
        "Same draw is reproducible from a seed",
        || {
