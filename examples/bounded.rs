...
fn main() {
    ...
    support::task_with_output("bounded(range) — half-open [0, range)", || {
        let mut rng = Random::from_u64_seed(1);
        let s: Vec<u32> = (0..8).map(|_| rng.bounded(100)).collect();
        vec![
            format!("samples = {s:?}"),
    ...
    support::task_with_output("random_range(min, max) — [min, max)", || {
    ...
    support::task_with_output("int(min, max) — i32 inclusive", || {
    ...
    support::task_with_output("uint(min, max) — u32 inclusive", || {
