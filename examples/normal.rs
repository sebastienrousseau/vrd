...
fn main() {
    ...
    support::task_with_output("Bulk mean and stddev approach the parameters", || {
        let mut rng = Random::from_u64_seed(0xBE11);
        let n = 50_000usize;
        let mu = 1.5;
        let sigma = 2.0;
    ...
    support::task_with_output("Tiny ASCII histogram of N(0, 1)", || {
        let mut rng = Random::from_u64_seed(7);
        let mut bins = [0u32; 21];
        for _ in 0..10_000 {
            let x = rng.normal(0.0, 1.0);
            let idx = ((x + 5.0) * 2.0).round().clamp(0.0, 20.0)
                as usize;
            bins[idx] += 1;
        }
