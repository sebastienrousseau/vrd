...
//!
//! - Altering the state of the PRNG: Demonstrates how seeding, twisting, and retrieving the MTI can be combined to control sequences of random numbers reproducibly.
//!
...
//!
//! - Serialization and deserialization: Demonstrates how to serialize and deserialize the random number generator state to/from JSON, incorporating error handling.
//!
...
fn demonstrate_vrd_error_usage() {
    ...
    runtime.block_on(async {
        match log_entry_async(log_entry).await {
            ...
            Err(e) => {
                if let Some(vrd_error) = e.downcast_ref::<VrdError>() {
                    match vrd_error {
                        ...
                        VrdError::GeneralError(msg) => println!(
    ...
    fn generate_positive_random(
        rng: &mut Random,
    ) -> Result<u32, VrdError> {
        let value = rng.rand();
        if value == 0 {
            Err(VrdError::GeneralError(
                "Generated zero value".to_string(),
            ))
        } else {
            Ok(value)
        }
    ...
    match generate_positive_random(&mut rng) {
        Ok(value) => {
            println!("✅ Generated positive random value: {}", value)
        }
        Err(e) => match e {
            VrdError::GeneralError(msg) => {
                println!("🔴 Error generating random value: {}", msg)
            }
            VrdError::LogError(_) => {
                println!("🔴 Unexpected logging error")
            }
        },
    }

    // Example 4: Error propagation with ? operator
    println!("\n🦀 Error propagation with ? operator:");
    fn complex_random_operation(
        rng: &mut Random,
    ) -> Result<u32, Box<dyn Error>> {
        let value1 = generate_positive_random(rng)?;
        let value2 = generate_positive_random(rng)?;
        Ok(value1.wrapping_add(value2))
    }

    match complex_random_operation(&mut rng) {
        Ok(result) => {
        ...
        Err(e) => {
            if let Some(vrd_error) = e.downcast_ref::<VrdError>() {
                match vrd_error {
                    VrdError::GeneralError(msg) => println!("🔴 General error in complex operation: {}", msg),
                    VrdError::LogError(msg) => println!("🔴 Unexpected logging error in complex operation: {}", msg),
                }
            } else {
                println!("🔴 An unknown error occurred in complex operation: {}", e);
            }
        }
    }
}
...
/// Demonstrates shuffling and sampling using the `vrd` crate.
fn demonstrate_shuffling_and_sampling() {
    let mut rng = Random::from_entropy();
    let values = &[1, 2, 3, 4, 5];
    // ... Shuffling and Sampling Examples ...
    // Example of using the `rand_slice` function to generate a random subslice from a given slice.
    // This can be useful for selecting random samples from a larger dataset.
...
/// Demonstrates the serialization and deserialization of a random number generator (RNG) and a custom Mersenne Twister configuration.
...
/// The main function that demonstrates the usage of the `vrd` crate.
fn main() {
    // Demonstrate the usage of the `vrd` error handling mechanisms.
    demonstrate_vrd_error_usage();

    // Demonstrates basic number generation using the `vrd` crate.
    demonstrate_basic_number_generation();

    // Demonstrates shuffling and sampling using the `vrd` crate.
    demonstrate_shuffling_and_sampling();
