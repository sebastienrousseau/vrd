...
The crates.io tarball for `vrd v0.0.11` is signed by
...
EXPECTED=$(curl -s https://crates.io/api/v1/crates/vrd/0.0.11 | jq -r '.version.checksum')
LOCAL=$(shasum -a 256 ~/.cargo/registry/cache/index.crates.io-*/vrd-0.0.11.crate | cut -d' ' -f1)
...
```bash
docker pull ghcr.io/sebastienrousseau/vrd:0.0.11
cosign verify \
    --certificate-identity-regexp 'https://github.com/sebastienrousseau/vrd/' \
    --certificate-oidc-issuer 'https://token.actions.githubusercontent.com' \
    ghcr.io/sebastienrousseau/vrd:0.0.11
```
