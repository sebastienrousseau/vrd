...
## [Unreleased] - v0.0.12 - Repo template alignment + 100% doc coverage

Mirrors noyalib's repo conventions inside vrd's single-crate layout.
No public API changes; all source-level diffs are doc comments and
private-item annotations. Operational scaffolding only.

### Added

- **`doc/` directory** (long-form references): `ARCHITECTURE.md`,
  `TESTING.md`, `COMPARISON.md` (feature matrix vs `rand`,
  `fastrand`, `oorandom`, `nanorand`, `rand_xoshiro`),
  `POLICIES.md` (MSRV / SemVer / output-stability / unsafe /
  supply-chain), `USER-GUIDE.md`, `MIGRATION-FROM-RAND.md`,
  `MIGRATION-FROM-FASTRAND.md`. `BENCHMARKS.md` moved into
  `doc/`; the root file becomes a stub.
- **Governance files**: `SECURITY.md` (RNG threat model, CSPRNG
  guidance, 14-day coordinated disclosure timeline),
  `GETTING_STARTED.md`, `GLOSSARY.md` (RNG terminology),
  `PLAN.md` (multi-phase roadmap through v1.0).
- **Supply-chain compliance**: `REUSE.toml` (REUSE 3.0
  annotations), `about.toml` + `about.hbs` (cargo-about
  pipeline producing `NOTICE`), `rust-toolchain.toml` (stable
  channel pin + rustfmt + clippy components), `codecov.yml`
  (mirrors `.tarpaulin.toml` excludes; `project.target: auto`
  with 1 % threshold; `patch.target: 100 %`).
- **`fuzz/` directory** with six libFuzzer targets via
  cargo-fuzz: `fuzz_xoshiro_seed`, `fuzz_mt_seed`,
  `fuzz_pcg_seed`, `fuzz_fill_array`, `fuzz_uniform_range`,
  `fuzz_quasirandom`. Local-only via `make fuzz-quick`
  (60 s/target) and `make fuzz-deep` (24 h/target); no CI
  cron.
- **`pkg/` packaging scaffolds** (subset of noyalib's
  11-channel matrix; the rest defer to v0.1.0):
  `pkg/homebrew/vrd.rb` (formula template),
  `pkg/docker/Dockerfile` (distroless multi-stage),
  `pkg/docker/Dockerfile.alpine` (musl-static),
  `pkg/nix/flake.nix` + `pkg/nix/package.nix`.
  `pkg/README.md` carries the channel matrix; `PUBLISH.md`
  the per-channel runbook; `VERIFY.md` the cosign + SLSA
  verification cookbook.
- **`scripts/` helpers**: `coverage-gap-report.sh`,
  `msrv.sh`, `pgo.sh`, `crush.sh` (PractRand wrapper).
- **`examples/distribution.rs`** demos the `Distribution<T>`
  trait surface added in v0.0.11 (#84).
- **CI**: `.github/workflows/docs.yml` rewritten as a
  two-job build-and-deploy pipeline; `scorecard.yml`
  (weekly OpenSSF), `security.yml` (Trivy fs scan).
- **GitHub Pages on Actions** (replaces the legacy `gh-pages`
  branch source). Every push to `main` rebuilds
  `doc.vrdlib.com`; every published release re-renders
  against the tagged source.
- **`RELEASE-NOTES-v0.0.12.md`** with the noyalib release-notes
  template.

### Changed

- **Makefile** replaced with a 30-target POSIX version
  (tab-indented, `.PHONY`-complete, single-line targets per
  noyalib). Default `all = check + clippy + test`. New
  `examples` target loops every example with `[ok]/[fail]`.
- **README.md** front matter aligned with noyalib: SPDX
  comment, centred SVG logo, h1, tagline, 5 for-the-badge
  badges (Actions, crates.io, docs.rs, lib.rs, OpenSSF
  Scorecard - the Codecov badge moves to BENCHMARKS), `---`
  divider, `## Contents` nav outline, sections reordered
  to noyalib's pattern.
- **`src/lib.rs`** flips `#![warn(missing_docs)]` to
  `#![deny(missing_docs)]` (lib-scoped). Adds
  `#![warn(clippy::missing_docs_in_private_items)]`. The
  docs.yml CI gate runs with `-D warnings`, upgrading
  both to hard failures.
- **GitHub repo description** rewritten to call out every
  v0.0.11 backend; 20 topics refreshed to include `csprng`,
  `pcg`, `chacha20`, `simd`, `quasi-random`, `halton`,
  `sobol`, `ziggurat`, `webassembly`.
- **Em dashes** (`-`) replaced with hyphens across all docs
  and source comments for a more direct prose style.

### Fixed

- **Private-item doc gaps** (37 across `build.rs`,
  `src/chacha.rs`, `src/distribution.rs`, `src/pcg.rs`,
  `src/quasirandom.rs`, `src/random.rs`, `src/xoshiro.rs`,
  `src/xoshiro_simd.rs`, `src/ziggurat.rs`). Every private
  field, function, method, and constant now carries a doc
  comment. `cargo clippy --all-features --no-deps -- -W
  clippy::missing_docs_in_private_items` → 0 warnings.
- **Trivy supply-chain compromise advisory**
  (`aquasecurity/trivy-action@0.28.0`) replaced with a SHA
  pin to `v0.36.0`.

## [0.0.11] - 2026-05-28 - Performance follow-ups + v0.1.0 differentiators
