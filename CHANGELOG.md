...
- **README competitive positioning** — replaced the `When to reach for vrd` hedge-paragraph with a feature comparison table covering `vrd`, `rand` 0.10, `fastrand` 2.x, and `oorandom` 11.x across 12 dimensions. New `What you don't have to depend on` callout names the four companion crates (`uuid`, `hex`/`data-encoding`, `base64`, `rand_distr`) that vrd makes optional. Added two FAQ entries: WebAssembly usage (entropy seeding caveat for default `wasm32-unknown-unknown`) and output stability commitment (bit-stable across patch releases; algorithm changes flagged in the `Migration` section of the `CHANGELOG`).
